...
# require(Rtsne)
# all.dists2 <- as.matrix(dist(A))
# set.seed(0)
# tsne.c<-Rtsne(all.dists2, is_distance=TRUE)
# par(mfrow=c(1,1))
# plot(tsne.c$Y[,1],tsne.c$Y[,2], pch=16,col=1,main="tsne")

#S<-princomp(A1)
#plot(S$scores[,1],S$scores[,2], pch=16,col=1,main="pca")
...
colnames(A1)<-c(1:D)
###
components <- sample(1:3,prob=c(0.65,0.3,0.05),size=N,replace=TRUE) #c(0.85,0.5,0.05)
