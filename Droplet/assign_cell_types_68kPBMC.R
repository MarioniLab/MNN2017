...
pbmc.tsne <- read.table("~/CI_filesystem/mnt/scratcha/jmlab/morgan02/10X/MNN/data/PBMC/PBMC_meta.tsv",
                        h=T, sep="\t", stringsAsFactors=FALSE)
...
m.exprs <- t(pbmc[m.ensembl, 1:(dim(pbmc)[2]-1)])
...
# 
# ggsave(comm.tsne,
#        filename="~/Dropbox/MNNfigures/10X_data/PBMC/PBMC_68k_community-tSNE.png",
#        width=6.75, height=4.75, dpi=300)

# ggplot(pbmc.merge[pbmc.merge$Community %in% c(1:15), ],
#       aes(x=Dim1, y=Dim2, colour=`TBX21`)) +
